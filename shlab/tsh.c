...
void eval(char* cmdline)
{
    ...
    bg = parseline(buf, argv);  //解析命令行，返回给argv数组
    ...
    if (!builtin_cmd(argv))//如果是内置命令那么结束，否则创建新的子进程
    {
        ...
        {
            sigprocmask(SIG_UNBLOCK, &pre_mask, NULL);//解除阻塞
            setpgid(0, 0);//保证是一个独立的进程，防止把shell一起kill掉
...
void sigchld_handler(int sig)
{
    ...
    int status;
    //仿照实验说明，给予一定的输出
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {//如果进入此循环，说明至少有一个子进程改变状态
        if (WIFEXITED(status)) {
            //正常退出    
            sigprocmask(SIG_BLOCK, &mask, &pre_mask);
            deletejob(jobs, pid);
            sigprocmask(SIG_SETMASK, &pre_mask, NULL);//解锁   
        }
...
void sigint_handler(int sig) {
    int olderrno = errno;//根据p536 G2要求
    pid_t fpid;
    //sigset_t mask, pre_mask;//定义信号集
    //sigfillset(&mask);//将其初始化
    //访问全局结构，为了安全，加锁
    //sigprocmask(SIG_BLOCK, &mask, &pre_mask);
    fpid = fgpid(jobs);//获取前台的pid
    //sigprocmask(SIG_SETMASK, &pre_mask, NULL);//解锁

    if (fpid != 0) {
        kill(-fpid, SIGINT);    //给前台进程及其子进程发送
    }
    errno = olderrno;
    return;
}
...
void sigtstp_handler(int sig)
{
    ...
    if (fpid != 0) {
        kill(-fpid, SIGTSTP);    //给前台进程及其子进程发送
    }
    errno = olderrno;
    return;
}
